...
export default function TemplateManagerPage() {
  ...
  const validateImportData = (
    ...
    if (template.exercises) {
      template.exercises.forEach(
        (exercise: TemplateExercise, index: number) => {
          if (!exercise.exerciseName) {
            errors.push(`Exercise ${index + 1} is missing a name`);
          }
          if (!Array.isArray(exercise.sets) || exercise.sets.length === 0) {
            warnings.push(
              `Exercise "${exercise.exerciseName || index + 1}" has no sets defined`,
            );
          }
        },
      );
    ...
    if (existingTemplate) {
      warnings.push(
        `A template named "${template.name}" already exists. Import will create a copy with a modified name.`,
      );
    }
  ...
  const importTemplate = async (file: File) => {
    ...
    try {
      ...
      for (const exportedExercise of data.exercises || []) {
        try {
          ...
          const existingExercise = searchResults.find(
            (ex) =>
              ex.name.toLowerCase() ===
              exportedExercise.exerciseName.toLowerCase(),
          );
      ...
      const updatedExercises: TemplateExercise[] = data.template.exercises.map(
      ...
      const createdTemplate =
        await templateRepository.createTemplate(newTemplate);
      ...
      setImportResult({
        success: false,
        errors: [
          "Failed to parse file. Please ensure it's a valid template export file.",
        ],
      });
    } finally {
      setIsLoading(false);
                    ...
                    <div className="p-3 bg-muted rounded-lg">
                      <div className="flex items-start gap-2">
                        ...
                        <div className="text-sm">
                          ...
                          <p className="text-muted-foreground">
                            Exported templates include exercise definitions and
                            can be shared with other users. Recipients can
                            import them directly into their template library.
                          </p>
                        </div>
                      </div>
                    </div>
          ...
          <TabsContent value="bulk" className="space-y-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Export multiple templates at once or import template
                  collections
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-6">
                {templates.length > 0 ? (
                    ...
                    <div className="p-3 bg-muted rounded-lg">
                      <div className="flex items-start gap-2">
                        ...
                        <div className="text-sm">
                          ...
                          <p className="text-muted-foreground">
                            Export multiple templates in one file for easy
                            sharing with coaching clients, backup purposes, or
                  ...
                  <div className="text-center py-8">
                    ...
                    <p className="text-muted-foreground">
                      No templates available for bulk operations
                    </p>
                    <p className="text-sm text-muted-foreground">
                      ...
                      <div className="flex-1">
                        ...
                        {importResult.template && (
                          <p className="text-sm text-green-700 mt-1">
                            Template &quot;{importResult.template.name}&quot;
                ...
                <div className="p-3 bg-muted rounded-lg">
                  <div className="flex items-start gap-2">
                    ...
                    <div className="text-sm">
                      ...
                      <p className="text-muted-foreground">
                        Import JSON files exported from this app. Missing
                        exercises will be created automatically.
                      </p>
                    </div>
                  </div>
                </div>
