  ...
  useEffect(() => {
    const loadExerciseProgress = async () => {
      ...
      try {
        ...
        const exercisePRs = await personalRecordRepository.getExerciseRecords(
          user.id,
          selectedExercise,
        );
        const maxWeightPRs = exercisePRs.filter(
          (pr) => pr.recordType === "max_weight",
        );

        const progressData: ProgressData[] = maxWeightPRs
          .map((pr) => ({
          ...
          <Card>
            <CardContent className="pt-6 text-center">
              ...
              <p className="text-muted-foreground mb-6">
                Complete a few workouts to unlock powerful analytics including
                personal records, progress charts, volume trends, and strength
                gains over time.
              </p>
  ...
  const favoriteExercisesData = workoutStats.favoriteExercises.map((ex) => ({
    name:
      ex.exerciseName.length > 15
        ? ex.exerciseName.substring(0, 15) + "..."
        : ex.exerciseName,
    value: ex.count,
          ...
          <TabsContent value="workout-streaks" className="space-y-4">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Your workout consistency patterns over time
                </CardDescription>
              </CardHeader>
              <CardContent>
                {allStreaks.length > 0 ? (
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <div className="font-medium">
                                {streak.workoutCount} day
                                {streak.workoutCount !== 1 ? "s" : ""} streak
                              </div>
                              <div className="text-sm text-muted-foreground">
                                {new Date(
                                  streak.startDate,
            ...
            <Card>
              ...
              <CardContent>
                {favoriteExercisesData.length > 0 ? (
                  ...
                  <div className="text-center py-12">
                    <p className="text-muted-foreground">
                      Complete more workouts to see your favorite exercises
                    </p>
                  </div>
                )}
              </CardContent>
