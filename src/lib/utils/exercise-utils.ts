...
export function normalizeMuscleGroups(muscles: string[]): string[] {
  return muscles.map(muscle => {
    ...
    return STANDARD_MUSCLE_GROUPS[normalized as keyof typeof STANDARD_MUSCLE_GROUPS] || 
 ...
 */
export function normalizeEquipment(equipment: string | undefined): string | undefined {
  if (!equipment) return undefined;
  
  const normalized = equipment.toLowerCase().trim();
  return STANDARD_EQUIPMENT[normalized as keyof typeof STANDARD_EQUIPMENT] || 
 ...
 */
export function normalizeExercise<T extends { muscles: string[]; equipment?: string }>(exercise: T): T {
  return {
    ...exercise,
...
export const EXERCISE_TRANSLATIONS = {
  ...
  'elevação lateral unilateral na polia média': 'Cable Lateral Raises - Single Arm',
  ...
  'puxada no pulley - pegada neutra aberta (barra romana)': 'Lat Pulldown - Neutral Grip (Roman Bar)',
  'puxada no pulley - pegada pronada aberta': 'Lat Pulldown - Wide Pronated Grip',
  'remada articulada unilateral - pegada neutra': 'Single Arm Machine Row - Neutral Grip',
  'remada baixa - pegada pronada aberta': 'Seated Cable Row - Wide Pronated Grip',
...
export function translateExerciseName(name: string): string {
  ...
  return EXERCISE_TRANSLATIONS[normalized as keyof typeof EXERCISE_TRANSLATIONS] || name;
}