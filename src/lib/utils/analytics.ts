...
export function generateVolumeProgressData(sessions: WorkoutSession[]): ProgressData[] {
  return sessions
    .filter(session => session.endedAt)
    .map(session => {
      const totalVolume = session.exercises.reduce((sum, exercise) => 
        sum + exercise.sets
          .filter(set => set.completedAt && set.weight && set.reps && !set.isWarmup)
...
export function generateWorkoutFrequencyData(sessions: WorkoutSession[], days: number = 30): ProgressData[] {
  ...
  sessions
    .filter(session => {
      ...
      return sessionDate >= startDate && sessionDate <= endDate && session.endedAt;
    })
    .forEach(session => {
...
export function generateStrengthProgressData(prs: PersonalRecord[], recordType: PersonalRecord['recordType'] = 'max_weight'): ProgressData[] {
  return prs
...
export function calculateWorkoutStreaks(sessions: WorkoutSession[]): { current: number; longest: number; streaks: Array<{ start: string; end: string; count: number }> } {
  ...
  let currentStreak = { start: workoutDates[0], end: workoutDates[0], count: 1 };
  ...
  for (let i = 1; i < workoutDates.length; i++) {
    ...
    if (daysDiff <= 1) {
      ...
      currentStreak = { start: workoutDates[i], end: workoutDates[i], count: 1 };
    }
  }
  ...
  const daysSinceLastWorkout = Math.floor((new Date(today).getTime() - new Date(lastWorkout).getTime()) / (1000 * 60 * 60 * 24));
...
export function getExerciseProgressTrend(data: ProgressData[]): 'improving' | 'declining' | 'stable' | 'insufficient_data' {
...
export function calculatePersonalBests(prs: PersonalRecord[]): {
  ...
  const maxWeight = prs
    ...
    .reduce((best, current) => !best || current.value > best.value ? current : best, null as PersonalRecord | null);
