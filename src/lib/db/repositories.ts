...
export const exerciseRepository = {
  ...
  async updateExercise(
    ...
    const normalizedUpdates =
      updates.muscles || updates.equipment
        ? normalizeExercise(
            updates as Partial<Exercise> & {
              muscles: string[];
              equipment?: string;
            },
          )
...
export const sessionRepository = {
  ...
  async finishSession(id: string): Promise<void> {
    ...
    if (session) {
      ...
      await workoutStreakRepository.updateStreakForWorkout(
        session.userId,
        session.date,
      );
    }
...
export const templateRepository = {
  ...
  async createTemplate(
    template: Omit<WorkoutTemplate, "createdAt" | "updatedAt">,
  ): Promise<WorkoutTemplate> {
    const now = new Date();
    const newTemplate = {
      ...template,
  ...
  async createTemplateFromSession(
    ...
    const templateExercises: TemplateExercise[] = session.exercises.map(
      (sessionEx, index) => ({
        id: crypto.randomUUID(),
...
export const personalRecordRepository = {
  ...
  async createRecord(
    record: Omit<PersonalRecord, "createdAt">,
  ): Promise<PersonalRecord> {
    const newRecord = {
      ...record,
      createdAt: new Date(),
  ...
  async checkForNewRecord(
    ...
    const maxWeightRecord = existingRecords.find(
      (r) => r.recordType === "max_weight",
    );
    const maxRepsRecord = existingRecords.find(
      (r) => r.recordType === "max_reps",
    );
    const maxVolumeRecord = existingRecords.find(
      (r) => r.recordType === "max_volume",
    );
    ...
    if (setData.weight && setData.reps) {
      ...
      const sameWeightRecords = existingRecords.filter(
        (r) => r.weight === setData.weight,
      );
      const maxRepsAtWeight = Math.max(
        ...sameWeightRecords.map((r) => r.reps || 0),
        0,
      );
      ...
      const best1RMRecord = existingRecords.find(
        (r) => r.recordType === "best_estimated_1rm",
      );
...
export const bodyMeasurementRepository = {
  ...
  async createMeasurement(
    measurement: Omit<BodyMeasurement, "createdAt">,
  ): Promise<BodyMeasurement> {
...
export const workoutStreakRepository = {
  ...
  async updateStreakForWorkout(
    ...
    if (currentStreak) {
      ...
      const daysDiff = Math.floor(
        (workoutDay.getTime() - streakEndDate.getTime()) /
          (1000 * 60 * 60 * 24),
      );
  ...
  async createStreak(
    streak: Omit<WorkoutStreak, "createdAt" | "updatedAt">,
  ): Promise<WorkoutStreak> {
    const now = new Date();
    const newStreak = {
      ...streak,
...
export const analyticsRepository = {
  async getExerciseProgress(
    ...
    // Calculate stats
    const totalSets = relevantSessionExercises.reduce(
      (sum, se) =>
        sum + se.sets.filter((set) => set.completedAt && !set.isWarmup).length,
      0,
    );

    const totalVolume = relevantSessionExercises.reduce(
      (sum, se) =>
        sum +
        se.sets
          .filter(
            (set) => set.completedAt && !set.isWarmup && set.weight && set.reps,
          )
    ...
    const estimated1RM =
      bestSet && bestSet.weight && bestSet.reps
        ? bestSet.weight * (36 / (37 - bestSet.reps))
        : 0;

    const lastSession = sessions
      .filter((s) =>
        relevantSessionExercises.some((se) => se.sessionId === s.id),
      )
      .sort(
    ...
    // Get recent PRs
    const recentPRs = await personalRecordRepository.getExerciseRecords(
      userId,
      exerciseId,
    );
    ...
    return {
      ...
      totalSessions: new Set(relevantSessionExercises.map((se) => se.sessionId))
        .size,
      totalSets,
      totalVolume,
      maxWeight,
  ...
  async getWorkoutStats(userId: string): Promise<WorkoutStats> {
    ...
    const currentStreak =
      await workoutStreakRepository.getCurrentStreak(userId);
    ...
    sessions.forEach((s) =>
      s.exercises.forEach((ex) => {
        const current = exerciseCount.get(ex.exerciseId) || {
          name: ex.nameAtTime,
          count: 0,
        };
        exerciseCount.set(ex.exerciseId, {
          name: current.name,
